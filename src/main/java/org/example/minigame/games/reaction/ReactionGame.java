...
public class ReactionGame implements MinigameBase {
    private static final long SUCCESS_THRESHOLD = 300; // 0.3초 이내 클릭 시 성공
    private static final long MIN_WAIT_TIME = 1000;    // 최소 대기 1초
    private static final long MAX_WAIT_TIME = 3000;    // 최대 대기 3초
    ...
    private void onTargetClick() {
        if (finished || hasResponded) return;
        hasResponded = true;

        if (!view.getTargetButton().isVisible()) {
            handleEarlyFail();
            return;
        }

        processResponse(System.currentTimeMillis());
    }

    private void processResponse(long eventTime) {
        reactionTime = eventTime - buttonAppearTime;
        finished = true;
    ...
    private void handleEarlyFail() {
        finished = true;
        success = false;
        stopTimer();
        view.getTargetButton().setVisible(false);
        view.getStatusLabel().setText("TOO EARLY!");
        finishGame();
        broadcastState();
    }
    ...
    /**
     * 키보드 입력(스페이스) 처리: 한 라운드당 첫 입력만 인정
     */
    private void registerKeyHandler() {
        Scene scene = view.getGameStage().getScene();
        if (scene == null) return;

        scene.setOnKeyPressed(event -> {
            if (hasResponded) return; // 연타/홀드 입력 무시
